...
## Introduction
This week I spent most of my time implementing a solution for [SIIM-ACR Pneumothorax Segmentation](kaggle.com/c/siim-acr-pneumothorax-segmentation/submissions) Kaggle competition and today's blog post is about the same. 

This blog post assumes the reader to have some idea about Image Segmentation, U-net and PyTorch and doesn't serve as an introduction to these. Rather, this blog post is code heavy and we build a solution that is then able to get top-100 position in [SIIM-ACR Pneumothorax Segmentation](kaggle.com/c/siim-acr-pneumothorax-segmentation/submissions) Kaggle compeition.

If you want to skip the explainations and get straight to training your models, a working notebook can be found [here](). 

So, let's get started!

## Problem Statement
From the description on Kaggle for this competition: 
...
Specifically, this is an Image Segmentation based problem where given a chest X-ray, the idea is to predict where Pneumothorax exists and also specify the position of Pneumothorax.

![](/images/acr_intro.png "fig-1 Sample Image")

Given an input image in `fig-1` (left), the idea is then to predict the mask of Pneumothorax in `fig-1` (right).
