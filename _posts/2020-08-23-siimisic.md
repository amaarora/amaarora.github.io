...
Many participants also found `EfficientNets` to work best for this competition - particularly an ensemble of them. We also tried `SeResnext-50`, `DenseNet`, `MobileNet` model architectures, but found `EfficientNets` to have the highest CV score.
...
However, We found BCE Loss to work well with Label Smoothing. Therefore, half of the models were trained using Focal Loss and half were trained with BCE Label Smoothing loss. 
...
## Class Imbalance
To deal with class imbalance, we used two strategies: 
- Train models with weighted focal loss instead of BCE loss 
- Add positive class labels using external data

This helped us deal with the extreme class imbalance. We also found bigger model architectures were more robust towards class imbalance than smaller architectures.