...
As can be seen from `fig-7`, the `MLP` consists of two layers followed by GELU non-linearity. The input and output dimensions remain the same for the `MLP` layer.
