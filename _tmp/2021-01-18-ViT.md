...
The first layer of the **Transformer Encoder** accepts **combined embeddings** of shape`197 x 768` as input. For all subsequent layers, the inputs are the outputs from the previous layer of the **Transformer Encoder**. There are a total of 12 such layers in the **Transformer Encoder**.

Next, these inputs - **combined embeddings**, are passed through a **Norm layer** (Layer Norm), and fed to **Multi-Head Attention** block. The inputs are first converted to `197 x 2304 (768*3)` shape using a **Linear layer** to get the **qkv** matrix. Next we reshape this **qkv** matrix into `3 x 197 x 768` where each of the three matrices of shape `197 x 768` represent the **q**, **k** and **v** matrices. These **q**, **k** and **v** matrices are further reshaped to `197 x 12 x 64` to represent the 12 attention heads. Once we have the **q**, **k** and **v** matrices, we finally perform the attention operation inside the **Multi-Head Attention** block which is given by the equation:

<img src="/images/attention.png" alt="Attention" width="400"/>

Once we get the outputs from the **Multi-Head Attention** block, these are added to the inputs as there are skip connections inside the **Transformer Encoder** block. 

These outputs are then fed to the **MLP** which consists of two linear layers and a GELU non-linearity to get the outputs from the **MLP** which are again added to the inputs (skip connection). This is the final output from one layer of the **Transformer Encoder**.
