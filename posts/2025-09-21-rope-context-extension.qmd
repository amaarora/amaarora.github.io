---
title: "How LLMs Scaled from 1K to 2M Context: A Technical Deep Dive"
subtitle: "Understanding APE, RoPE, Position Interpolation, NTK-Aware Scaling, Dynamic Scaling, and YaRN"
description: |
    A comprehensive technical guide through the evolution of positional encodings in Large Language Models since GPT-2 that helped scale context length from 1,024 to 2,000,000+ tokens with minimal fine-tuning.
categories:
...
When GPT-2 was released in 2019, it came with a context window of just 1,024 tokens. Fast forward to today, and we have model Grok-4-fast that was released with 2M context length! [@xai2025grok4fast]

The Gemini series was the first to allow accepting 1 million tokens. [@google2025gemini]

As part of this blog post, I want to enlighten the readers with some of the techniques that have made this possible and the evolution of these techniques since the GPT-2 days back in 2019. This is a highly technical blog post suitable for readers who are looking to deep dive on the subject.

Both model architecture and position embeddings have played a key role when it comes to long context inference. Traditionally, in the Transformer architecture released in 2017, the authors used Absolute Positional Embeddings or APE. These encode positional information into the attention mechanism through absolute positions which get added to the vector representations of the embeddings.

A comprehensive survey of the techniques is available in YaRN [@yarn2023].

Let's look at all of them with the help of their PyTorch implementations.
