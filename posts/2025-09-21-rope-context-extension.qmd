...
When the original Transformer was introduced in 2017, it came with a context window of just 512 tokens. Fast forward to today, and we have model Grok-4-fast that was released with 2M context length! [@xai2025grok4fast] The Gemini series was the first to allow accepting 1 million tokens. [@google2025gemini]

This 4000x improvement didn't come from training larger models or using more data - it came from fundamental advances in how models encode position. Many of these breakthroughs emerged from the open source community, with key discoveries documented in blog posts and forums before being formalized in academic papers.

In this blog post, we'll examine the evolution of positional encodings - from the Transformer's sinusoidal functions (APE) to RoPE's rotational matrices, from Position Interpolation to Dynamic Scaling. We'll implement each technique, visualize the underlying mathematics, and understand the critical insight that models memorize positional patterns rather than learning generalizable representations.

This is a highly technical deep dive with working PyTorch code throughout. By the end, you'll understand exactly how modern LLMs can process entire books, why they catastrophically fail at position 1025, and how the community solved problems that seemed mathematically impossible.
